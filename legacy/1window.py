"""Single-window prototype with a desktop alert.
...
while True:
    ...
    for *xyxy, conf1, class_id1 in people_results.boxes.data.tolist():
        if int(class_id1) == 0:
            ...
            cv2.putText(frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 215, 0), 2)
    ...
    for *abab, conf2, class_id2 in gun_results.boxes.data.tolist():
        ...
        cv2.putText(frame, label2, (a1, b1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 215), 2)
        alert = True
    ...
    for *jkjk, conf3, class_id3 in fire_results.boxes.data.tolist():
        ...
        cv2.putText(frame, label3, (j1, k1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (215, 0, 0), 2)
        alert = True

    # Resize the output frame to a comfortable size
    output_frame = cv2.resize(frame, (width, height))
    # Display the frame
    cv2.imshow('YOLOv11 Detection', output_frame)
