"""Quad-panel prototype: raw, person, weapon, fire.
...
while True:
    ...
    # One copy per panel. `a = b = c = frame` binds three names to a
    # single buffer, so every panel inherited the previous panel's boxes.
    pd_frame, gd_frame, fd_frame = frame.copy(), frame.copy(), frame.copy()
    ...
    canvas[:raw_frame.shape[0], :raw_frame.shape[1]] = raw_frame
    ...
    for *xyxy, conf1, class_id1 in people_results.boxes.data.tolist():
        if int(class_id1) == 0:
            ...
            cv2.putText(pd_frame, "People", (8, 24), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 215, 0), 2)
            cv2.putText(pd_frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 215, 0), 2)
    pd_frame = cv2.resize(pd_frame, (width // 2, height // 2))
    canvas[:pd_frame.shape[0], width // 2:width // 2 + pd_frame.shape[1]] = pd_frame
    ...
    canvas[height // 2:height // 2 + gd_frame.shape[0], :gd_frame.shape[1]] = gd_frame

    # Apply YOLOv11 object detection for detecting fire
    fire_results = fire_model(fd_frame, conf=0.5)[0]

    # Draw bounding boxes and labels
    for *jkjk, conf3, class_id3 in fire_results.boxes.data.tolist():
        j1, k1, j2, k2 = int(jkjk[0]), int(jkjk[1]), int(jkjk[2]), int(jkjk[3])
        label3 = '{} {}%'.format(fire_model.names[int(class_id3)], int(conf3 * 100))
    ...
    canvas[height // 2:height // 2 + fd_frame.shape[0], width // 2:width // 2 + fd_frame.shape[1]] = fd_frame
